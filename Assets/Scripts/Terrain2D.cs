...
public class Terrain2D : MonoBehaviour
{
	...
	protected virtual void PositionChildren() {
		Terrain2DChild[] c = transform.GetComponentsInChildren<Terrain2DChild>();
	...
	public virtual void CreateGround () {
		CacheComponents();
		Init();
		SpriteSorting();
		Wave();
	...
	protected virtual void GenerateMeshColors() {
	...
	protected virtual void SpriteSorting() {
...
public class Terrain2DEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		tar.width = Mathf.Clamp(tar.width, 0, Mathf.Infinity);
		if (GUI.changed) {
			tar.CreateGround();
		}
		GUILayout.Space(10);
		...
		if (GUILayout.Button("Update All Grounds")) {
			Terrain2D[] z = FindObjectsOfType<Terrain2D>();
        ...
        EditorUtility.SetSelectedRenderState((target as Terrain2D).cacheMeshRenderer, 0);
	}


	protected virtual void OnEnable() {
		tar = (target as Terrain2D);
	...
	protected virtual void SmoothCurves() {
		Undo.RegisterCompleteObjectUndo(tar, "Smooth Curves");
	...
	protected virtual void OnSceneGUI() {
		...
		for (int i = 0; i < tar.terrainCurve.length; i++) {
			...
			//	Debug.Log("undoRegistered");
				undoRegistered = true;
				Undo.RegisterCompleteObjectUndo(tar, "Drag Point");
			...
			if(i > 0 && i < tar.terrainCurve.length-1) {
				...
				if (shiftDown && Handles.Button(v, Quaternion.identity, .8f * tar.handleScale, 1 * tar.handleScale, Handles.SphereHandleCap)) {
					Undo.RegisterCompleteObjectUndo(tar, "Delete Key");
			...
			if (ctrlDown && Handles.Button(v, Quaternion.identity, .8f * tar.handleScale, 1 * tar.handleScale, Handles.SphereHandleCap)) {
				Undo.RegisterCompleteObjectUndo(tar, "Smooth Key");
				tar.terrainCurve.SmoothTangents(i, 0f);
				tar.CreateGround();
				return;
			}
			...
			if (i < tar.terrainCurve.length - 1) {
				...
				if (Handles.Button(vvv2, Quaternion.identity, .4f * tar.handleScale, .4f * tar.handleScale, Handles.CubeHandleCap)) {
					Undo.RegisterCompleteObjectUndo(tar, "Create Key");
